class slideToUnlock {
  constructor(el, options) {
    ...
    this.settings = {
      ...
      status: false,
      width: 350,
      lockedHandleIcon: 'fas fa-chevron-right',
      unlockedHandleIcon: 'fas fa-check',
      dependant: ''
    }
  ...
  init() {
    this.$el.addClass('slideToUnlock').css('max-width', this.settings.width + 'px');
    ...
    this.$el.append("<div class='drag locked_handle '> <i class='" + this.settings.lockedHandleIcon + "'></i> </div>");
  ...
  touchStart(event = window.event) {
    ...
    const disabled = $('.slideToUnlock').hasClass('is-disabled');
  ...
  touchMove(event = window.event) {
    ...
    if (edge < this.leftEdge) {
      ...
      this.$drag
        .removeClass('unlocked_handle')
        .addClass('locked_handle')
        .find('i')
        .removeClass(this.settings.unlockedHandleIcon)
        .addClass(this.settings.lockedHandleIcon);
      this.start = false;
    ...
    if (edge > this.rightEdge - this.$drag.outerWidth()) {
      ...
      this.$drag
        .removeClass('locked_handle')
        .addClass('unlocked_handle')
        .find('i')
        .removeClass(this.settings.lockedHandleIcon)
        .addClass(this.settings.unlockedHandleIcon);
